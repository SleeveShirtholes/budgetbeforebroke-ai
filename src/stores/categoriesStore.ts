...
const useCategoriesStore = create<CategoriesState>((set, get) => ({
    ...
    setSelectedReplacementCategory: (category) => set({ selectedReplacementCategory: category }),
    ...
    editCategory: () => {
        ...
        set({
            categories: categories.map((cat) =>
                cat.id === categoryToEdit.id ? { ...cat, name: form.name, description: form.description } : cat
            ),
            isEditModalOpen: false,
            categoryToEdit: null,
        });
