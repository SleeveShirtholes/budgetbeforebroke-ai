...
function getMerchantForCategory(category: TransactionCategory): {
  name: string;
  location: string;
} {
  const categoryMerchants =
    merchants[category as keyof typeof merchants] || merchants.Shopping;
  return categoryMerchants[
    Math.floor(Math.random() * categoryMerchants.length)
  ];
}
