...
import { NavDropdownWithReactIcon, getNavigationData } from "@/utils/navigationLoader";
 ...
 */
export default function Header({ userAvatar = "/default-avatar.png", userName = "User" }: HeaderProps) {
    const [isUserDropdownOpen, setIsUserDropdownOpen] = useState(false);
    const [openDropdown, setOpenDropdown] = useState<string | null>(null);
    const [navigationData, setNavigationData] = useState<Record<string, NavDropdownWithReactIcon>>({});
                ...
                <div className="flex justify-between items-center h-16">
                    <div className="flex items-center">
                        ...
                        <nav className="ml-10 flex space-x-8">
                            {(Object.entries(navigationData) as [string, NavDropdownWithReactIcon][]).map(
                                ([key, dropdown]) => (
                                    <div key={key} className="relative" ref={(el) => setDropdownRef(el, key)}>
                                        <button
