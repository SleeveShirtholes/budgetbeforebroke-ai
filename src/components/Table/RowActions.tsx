...
export default function RowActions({ actions }: RowActionsProps) {
    ...
    return (
        <div className="relative inline-block" ref={actionRef} style={{ zIndex: isOpen ? 9999 : "auto" }}>
            <button
                onClick={() => setIsOpen(!isOpen)}
                className={`p-1.5 rounded-full transition-colors ${
                    isOpen ? "bg-secondary-100 text-secondary-800" : "hover:bg-secondary-100 text-secondary-600"
                }`}
            >
            ...
            {isOpen && (
                <div
                    ...
                    style={{
                        ...
                        zIndex: 9999,
                        width: "160px",
                        top: actionRef.current ? actionRef.current.getBoundingClientRect().bottom + 4 : 0,
                        right: actionRef.current
                            ? window.innerWidth - actionRef.current.getBoundingClientRect().right
                            : 0,
                    }}
                >
