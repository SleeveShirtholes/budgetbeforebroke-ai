...
describe("TableBody Component", () => {
    ...
    it("handles row expansion", () => {
        const detailPanel = (row: (typeof mockData)[0]) => <div>Details for {row.name}</div>;
    ...
    it("renders custom accessor content without highlighting", () => {
        const columnsWithAccessor: ColumnDef<(typeof mockData)[0]>[] = [
            ...columns,
            {
                key: "custom",
                header: "Custom",
                accessor: (row) => <span data-testid={`custom-${row.id}`}>Custom {row.name}</span>,
            },
        ];

        renderWithTable(<TableBody {...defaultProps} columns={columnsWithAccessor} searchQuery="Custom" />);
    ...
    it("handles data without id field", () => {
        ...
        renderWithTable(<TableBody {...defaultProps} data={dataWithoutId} columns={columnsWithoutId} />);
    ...
    it("handles search with highlighting across multiple columns", () => {
        ...
        const columnsWithDescription: ColumnDef<(typeof dataWithMultipleMatches)[0]>[] = [
            { key: "name", header: "Name" },
    ...
    describe("Chevron Icon for Expandable Rows", () => {
        ...
        it("renders chevron icon when detailPanel is provided", () => {
            ...
            const rows = screen.getAllByRole("row");
            const firstCellOfFirstDataRow = rows[0].querySelector("td:first-child svg");
