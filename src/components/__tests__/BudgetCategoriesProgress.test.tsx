...
describe("BudgetCategoriesProgress", () => {
    ...
    it("displays correct spending amounts and budgets", () => {
        ...
        mockCategories.forEach((category) => {
            expect(
                screen.getByText(`$${category.spent.toFixed(2)} / $${category.budget.toFixed(2)}`)
            ).toBeInTheDocument();
        });
    });
    ...
    it("applies correct styles and colors to progress bars", () => {
        const { container } = render(<BudgetCategoriesProgress categories={mockCategories} />);

        const progressBars = container.querySelectorAll(".absolute.h-full.rounded-full");
        progressBars.forEach((bar, index) => {
            const isOverspent = mockCategories[index].spent > mockCategories[index].budget;
