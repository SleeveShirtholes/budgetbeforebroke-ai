...
describe("CustomSelect", () => {
    ...
    it("maintains selected value when filtering and clearing", async () => {
        const { rerender } = render(<CustomSelect {...defaultProps} value="option2" clearable={true} />);
        ...
        rerender(<CustomSelect {...defaultProps} value="option2" clearable={true} />);
        expect(screen.getByPlaceholderText("Second Option")).toBeInTheDocument();
    });

    it("renders with left icon when provided", () => {
        render(<CustomSelect {...defaultProps} leftIcon={<CalendarIcon data-testid="calendar-icon" />} />);
        expect(screen.getByTestId("calendar-icon")).toBeInTheDocument();
    });

    it("adjusts padding when left icon is present", () => {
        const { container } = render(
            <CustomSelect {...defaultProps} leftIcon={<CalendarIcon data-testid="calendar-icon" />} />
        );
    ...
    it("shows the placeholder when value is empty string and no placeholder is provided", () => {
        render(<CustomSelect label="Test Select" options={mockOptions} value="" onChange={jest.fn()} />);
    ...
    it("does not show clear button when clearable is false (default)", async () => {
        render(<CustomSelect label="Test Select" options={mockOptions} value="option1" onChange={jest.fn()} />);
    ...
    it("does not show clear button when clearable is true but no value is selected", async () => {
        render(
            <CustomSelect label="Test Select" options={mockOptions} value="" onChange={jest.fn()} clearable={true} />
        );
        expect(screen.queryByRole("button")).not.toBeInTheDocument();
    });
