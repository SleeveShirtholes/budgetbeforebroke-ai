...
const categoryLiterals = ["Feature Request", "Issue", "General Question"] as const;
export type SupportCategory = (typeof categoryLiterals)[number];

// Then define the options for the select component
export const supportCategoriesOptions: SelectOption[] = categoryLiterals.map((cat) => ({
    value: cat,
    label: cat,
}));
...
export const supportStatusOptions: SelectOption[] = statusLiterals.map((status) => ({
    value: status,
    label: status,
}));
