...
function MockSupportFilters(props: Record<string, unknown>) {
    return (
        <div data-testid="support-filters">
            <button onClick={() => (props.onIssueViewChange as (v: string) => void)("public")}>
                All Public Issues
            </button>
            <button onClick={props.onCreateRequest as () => void}>Create New Support Request</button>
        </div>
    );
}
...
function MockSupportTableHeader(props: Record<string, unknown>) {
    return (
        <div data-testid="support-table-header">
            ...
            <button onClick={() => (props.onStatusViewChange as (v: string) => void)("closed")}>Closed</button>
        </div>
    );
}
...
describe("Support Page", () => {
    ...
    it("opens and closes the new request modal", () => {
        ...
        const heading = screen.getByRole("heading", { name: "Create New Support Request" });
        expect(heading).toBeInTheDocument();
        // Close modal
        fireEvent.click(screen.getByRole("button", { name: /cancel/i }));
        // Log visibility for debugging

        console.log("Modal heading visible after cancel:", heading.offsetParent !== null);
    ...
    it("creates a new support request and updates the table", async () => {
        ...
        fireEvent.change(screen.getByPlaceholderText("Briefly describe your issue"), {
            target: { value: "Test Title" },
        });
        fireEvent.change(screen.getByPlaceholderText("Provide a detailed description of the problem or request..."), {
            target: { value: "Test Description" },
        });
        // Submit new request
        fireEvent.click(screen.getByRole("button", { name: /submit/i }));
        // Modal should close
        await waitFor(() => expect(screen.queryByTestId("new-request-form")).not.toBeInTheDocument());
        // Table should have the new request
        expect(screen.getAllByTestId("support-table")[0].textContent).toContain("Test Title");
    });
    ...
    it("filters to public issues when tab is clicked", () => {
        ...
        expect(screen.getByTestId("support-table").textContent).toContain("Feature Request: Dark Mode");
        expect(screen.getByTestId("support-table").textContent).not.toContain("Login Issue");
    });
    ...
    it("filters to closed issues when status is changed", () => {
        ...
        expect(screen.getByTestId("support-table").textContent).toContain("Payment Failed");
        expect(screen.getByTestId("support-table").textContent).not.toContain("Login Issue");
    });
    ...
    it("does not add a new request if required fields are empty", () => {
        render(<Support />);
        // Count rows before
        const beforeRows = screen.getAllByTestId("support-table")[0].querySelectorAll("div").length;
        // Open modal
        fireEvent.click(screen.getByText("Create New Support Request"));
        // Simulate submitting with empty fields (the default newRequest is empty)
        fireEvent.click(screen.getByRole("button", { name: /submit/i }));
        // Count rows after
        const afterRows = screen.getAllByTestId("support-table")[0].querySelectorAll("div").length;
