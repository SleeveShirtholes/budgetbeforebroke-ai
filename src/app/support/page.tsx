...
const mockSupportRequests: SupportRequest[] = [
    {
        ...
        isPublic: false,
        comments: [{ id: "C1", user: "Admin", text: "Looking into this.", timestamp: "2024-07-28T10:00:00Z" }],
        upvotes: 10,
        downvotes: 1,
        user: "John Doe",
    },
...
export default function Support() {
    ...
    const [newRequest, setNewRequest] = useState<Omit<SupportRequest, "id" | "lastUpdated">>({
        title: "",
        description: "",
        category: "Issue",
        status: "Open",
    ...
    const columns: ColumnDef<SupportRequest>[] = [
        { key: "id", header: "ID", sortable: true, filterable: true },
        {
            ...
            filterable: true,
            accessor: (row) => (
                <span className="font-medium text-primary-600 hover:text-primary-700 cursor-pointer">{row.title}</span>
            ),
        },
    ...
    const handleUpvote = (requestId: string) => {
        setRequests((prevRequests) =>
            prevRequests.map((req) => (req.id === requestId ? { ...req, upvotes: req.upvotes + 1 } : req))
        );
    };
    ...
    const handleDownvote = (requestId: string) => {
        setRequests((prevRequests) =>
            prevRequests.map((req) => (req.id === requestId ? { ...req, downvotes: req.downvotes + 1 } : req))
        );
    };
