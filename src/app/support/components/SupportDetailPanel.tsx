...
const SupportDetailPanel: React.FC<SupportDetailPanelProps> = ({
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 my-2">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
            ...
            <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-gray-700">{row.upvotes}</span>
                    <Button
                        variant="text"
                        size="sm"
                    ...
                    <span className="text-sm font-medium text-gray-700">{row.downvotes}</span>
                </div>
            </div>
        </div>
        {/* Metadata Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 mb-6">
            <div>
                <span className="text-gray-500">ID:</span> <span className="font-mono text-gray-800">{row.id}</span>
            </div>
            <div>
            ...
            <div className="flex items-center gap-2">
                ...
                <CustomSelect
                    ...
                    value={row.status}
                    onChange={(newStatusValue) => onStatusChange(row.id, newStatusValue as SupportStatus)}
                    fullWidth={false}
                />
            </div>
            <div>
            ...
            <div>
                ...
                <span className="text-gray-800">{new Date(row.lastUpdated).toLocaleDateString()}</span>
            </div>
        </div>
        {/* Divider */}
        <hr className="my-4" />
        ...
        <div className="mt-8">
            ...
            <div className="space-y-4 mb-6">
                {row.comments.length > 0 ? (
                    row.comments.map((comment) => (
                        <div key={comment.id} className="bg-gray-50 border border-gray-200 rounded-md p-3">
            ...
            <div className="mt-6 bg-gray-50 border border-gray-200 rounded-md p-4">
                <h5 className="text-sm font-semibold text-gray-700 mb-2">Add a comment</h5>
                <TextArea
                    ...
                    label="Your Comment"
                />
                <Button variant="primary" size="sm" onClick={() => onAddComment(row.id)} className="mt-2">
                    Submit Comment
                </Button>
            </div>
        </div>
