...
jest.mock("@/components/Card", () => {
    const Card = ({ children, variant, padding }: { children: React.ReactNode; variant: string; padding: string }) => (
...
jest.mock("../components/SignInMethods", () => {
    const SignInMethods = ({ methods }: { methods: Array<{ type: string; provider: string }> }) => (
...
jest.mock("../components/AccountInformation", () => {
    const AccountInformation = ({ accountCreated, lastLogin }: { accountCreated: string; lastLogin: string }) => (
...
describe("ProfilePage", () => {
    ...
    it("handles avatar change", async () => {
        ...
        expect(avatar).toHaveAttribute("src", expect.stringContaining("url=%2Fnew-avatar.png"));
    });
