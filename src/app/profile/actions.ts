    ...
    try {
        ...
        if (name) {
            ...
            if (latestPasskey.length > 0) {
                await db.update(passkey).set({ name }).where(eq(passkey.id, latestPasskey[0].id));
            }
        }
        ...
        return {
            success: false,
            error: {
                message: error instanceof Error ? error.message : "An unexpected error occurred while adding passkey",
            },
        };
    }
}
...
export async function deletePasskey(passkeyId: string) {
    try {
        ...
        return {
            success: false,
            error: {
                message: error instanceof Error ? error.message : "An unexpected error occurred while deleting passkey",
            },
        };
    }
}
    ...
    try {
        ...
        if (!hasExistingPassword) {
            ...
            const credentialAccount = await db
                .select()
                .from(account)
                .where(and(eq(account.userId, user.id), eq(account.providerId, "credentials")))
                .limit(1);
        ...
        return {
            hasExistingPassword,
            message: hasExistingPassword ? "Password can be updated" : "Password can be created",
        };
    } catch (error) {
        ...
        const methods = await Promise.all(
            userAccounts.map(async (acc) => {
                ...
                return {
                    ...
                    provider: acc.providerId.charAt(0).toUpperCase() + acc.providerId.slice(1),
...
export async function deleteSignInMethod(methodId: string) {
    try {
        ...
        // Delete the account
        await db.delete(account).where(and(eq(account.id, methodId), eq(account.userId, user.id)));
    } catch (error) {
...
export async function getAccountInformation() {
    try {
        ...
        return {
            ...
            lastLogin: lastSession?.createdAt.toISOString() || userProfile.createdAt.toISOString(),
        };
    } catch (error) {
