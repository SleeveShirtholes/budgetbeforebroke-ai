...
import { getAccountInfo, getCurrentUserPasskeys, setPasswordAction, updatePasswordTimestamp } from "../actions";
...
const AccountSecurity = () => {
    ...
    // Fetch passkey data
    const { data: passkeys, mutate: mutatePasskeys } = useSWR<Passkey[]>("passkeys", getCurrentUserPasskeys);

    // Fetch account data
    const { data: account, mutate: mutateAccount } = useSWR("account", getAccountInfo);
     ...
     */
    const handleChangePassword = async (currentPassword: string | null, newPassword: string) => {
        ...
        try {
            ...
            if (!validationResult.success) {
                showToast(validationResult.error || "Failed to validate password change", { type: "error" });
                return;
            }
            ...
            if (validationResult.hasExistingPassword) {
                ...
                if (result && result.error) {
                    showToast(
                        typeof result.error === "string"
                            ? result.error
                            : (result.error as { message?: string })?.message || "Failed to change password",
                        { type: "error" }
                    );
                    return;
                }
                ...
                if (result && result.error) {
                    showToast(
                        typeof result.error === "string"
                            ? result.error
                            : (result.error as { message?: string })?.message || "Failed to set password",
                        { type: "error" }
                    );
                    return;
                }
                    ...
                    {passkeys.length > 0 ? (
                        <div className="divide-y divide-gray-200">
                            {passkeys.map((passkey) => (
                                <div key={passkey.id} className="px-4 py-3 flex items-center justify-between">
                                    <div>
