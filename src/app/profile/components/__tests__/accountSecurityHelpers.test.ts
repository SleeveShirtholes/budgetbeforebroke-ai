...
describe("accountSecurityHelpers", () => {
    ...
    describe("handleDeletePasskeyFn", () => {
        it("successfully deletes a passkey", async () => {
            ...
            expect(authClient.passkey.deletePasskey).toHaveBeenCalledWith({ id: mockPasskey.id });
            expect(mockShowToast).toHaveBeenCalledWith("Passkey deleted successfully", { type: "success" });
        ...
        it("handles error when deleting passkey", async () => {
            ...
            (authClient.passkey.deletePasskey as jest.Mock).mockRejectedValueOnce(error);
            ...
            expect(authClient.passkey.deletePasskey).toHaveBeenCalledWith({ id: mockPasskey.id });
            expect(mockShowToast).toHaveBeenCalledWith("Failed to delete passkey", { type: "error" });
        ...
        it("handles null passkey", async () => {
            ...
            expect(mockShowToast).toHaveBeenCalledWith("No passkey selected", { type: "error" });
            expect(mockMutatePasskeys).not.toHaveBeenCalled();
            expect(mockSetIsDeletePasskeyModalOpen).not.toHaveBeenCalled();
            expect(mockSetPasskeyToDelete).not.toHaveBeenCalled();
        });
