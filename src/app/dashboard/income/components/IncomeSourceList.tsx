 ...
 */
export function IncomeSourceList({ incomeSources, onEdit, onDelete, onAdd }: IncomeSourceListProps) {
    return (
        <Card>
            <div className="divide-y divide-gray-200">
                {incomeSources.length === 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center text-secondary-500">
                        ...
                        {/* Empty state heading */}
                        <div className="text-lg font-semibold text-secondary-900 mb-1">No income sources yet</div>

                        {/* Helpful description text */}
                        <p className="text-sm text-secondary-500 max-w-sm mb-4">
                            Add your first income source to start tracking your monthly budget. This could be your
                            salary, freelance work, or any other regular income.
                        </p>
                    ...
                    incomeSources.map((source) => (
                        <div key={source.id} className="py-4 flex items-center justify-between">
                            ...
                            <div>
                                {/* Income source name */}
                                <h3 className="text-lg font-medium text-secondary-900">{source.name}</h3>
                                ...
                                {source.notes && <p className="text-sm text-secondary-500 mt-1">{source.notes}</p>}
                            </div>

                            {/* Action buttons for this income source */}
                            <div className="flex space-x-2">
                                {/* Edit button */}
                                <Button variant="secondary" size="sm" onClick={() => onEdit(source)}>
                                    Edit
                                </Button>

                                {/* Delete button */}
                                <Button variant="danger" size="sm" onClick={() => onDelete(source.id)}>
                                    Delete
                                </Button>
                            </div>
                        </div>
