...
export default function PaycheckPlanningPage() {
  ...
  const handleDebtAllocated = async (
    ...
    try {
      ...
      // Batch refresh all data instead of individual calls
      if (selectedAccount?.id) {
        try {
          const [updatedAllocations] = await Promise.all([
            getDebtAllocations(selectedAccount.id),
            mutatePlanningData?.(),
            mutateAllocations?.(),
          ]);
  ...
  if (isAccountsLoading) {
    return (
      <div className="space-y-4">
        ...
        <div data-testid="spinner" className="hidden" />
      </div>
    );
  }

  // Loading states for planning data or hidden debts (only when account is selected)
  if (
    selectedAccount &&
    (isPlanningDataLoading || isHiddenDebtsLoading || isInitialLoading)
  ) {
    return (
      <div className="space-y-4">
        <div className="space-y-3">
        ...
        <Card className="p-6">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading paycheck planning data...</p>
          </div>
        </Card>
      </div>
    );
    ...
    <div className="space-y-4">
      ...
      <>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-0 sm:justify-center lg:sm:justify-end">
          ...
          <div className="relative w-full sm:w-auto" ref={dropdownRef}>
            <Button
              ...
              isLoading={isChangingPlanningWindow}
        ...
        <AssignmentBasedInterface
          ...
          isChangingPlanningWindow={isChangingPlanningWindow}
        />
