...
export default function WarningsPanel({ warnings }: WarningsPanelProps) {
  ...
  const severityOrder: Array<keyof typeof severityConfig> = ['high', 'medium', 'low'];
  const hasHighSeverity = warnings.some(w => w.severity === 'high');

  return (
    <Card className={`${hasHighSeverity ? 'border-red-200' : 'border-yellow-200'} border`}>
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          <ExclamationTriangleIcon className={`h-5 w-5 ${hasHighSeverity ? 'text-red-600' : 'text-yellow-600'}`} />
          <h3 className={`text-lg font-semibold ${hasHighSeverity ? 'text-red-900' : 'text-yellow-900'}`}>
            Payment Warnings
          </h3>
          ...
          {severityOrder.map((severity) => {
            ...
            return (
              <div key={severity} className="space-y-2">
                <h4 className={`text-sm font-medium ${config.titleColor} capitalize`}>
                      ...
                      <div className="flex-1">
                        ...
                        {(warning.debtId || warning.paycheckId) && (
                          <div className="mt-1 text-xs text-gray-500">
                            {warning.type === 'debt_unpaid' && 'Consider adjusting payment schedule or increasing income'}
                            {warning.type === 'insufficient_funds' && 'Consider postponing non-essential payments'}
                            {warning.type === 'late_payment' && 'Consider paying early or adjusting due dates'}
                          </div>
                        )}
                      </div>
                    </div>
        ...
        <div className="pt-3 border-t border-gray-200">
          <div className="text-sm text-gray-600">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              {hasHighSeverity && (
                <>
                  <li>Review your income sources and consider adding additional sources</li>
                  <li>Prioritize essential debt payments and consider renegotiating due dates</li>
                </>
              )}
              {groupedWarnings.medium && (
                <li>Consider adjusting payment timing to align better with paycheck dates</li>
              )}
              <li>Set up automatic payments a few days after each paycheck</li>
              <li>Build an emergency fund to handle unexpected payment timing issues</li>
            </ul>
          </div>
        </div>
      </div>
