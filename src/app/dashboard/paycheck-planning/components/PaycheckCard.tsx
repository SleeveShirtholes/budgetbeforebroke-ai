...
import { CurrencyDollarIcon, CalendarDaysIcon, CheckCircleIcon, ExclamationTriangleIcon } from "@heroicons/react/24/outline";
...
export default function PaycheckCard({ allocation, paycheck }: PaycheckCardProps) {
      ...
      <div className="space-y-4">
        ...
        {allocation.allocatedDebts.length > 0 && (
          <div className="space-y-2">
            <h4 className="text-sm font-medium text-gray-700">Allocated Payments</h4>
            <div className="space-y-2">
        ...
        <div className={`flex items-center justify-between p-4 rounded-lg ${
          ...
          <span className={`text-lg font-bold ${
            ...
            {hasInsufficientFunds ? '-' : ''}${Math.abs(allocation.remainingAmount).toLocaleString()}
          </span>
        </div>
        ...
        <div className="text-xs text-gray-500 border-t pt-3">
          <div className="flex justify-between">
            <span>Allocated to {allocation.allocatedDebts.length} payment{allocation.allocatedDebts.length !== 1 ? 's' : ''}</span>
            <span>
              ${allocation.allocatedDebts.reduce((sum, debt) => sum + debt.amount, 0).toLocaleString()} total
            </span>
          </div>
        </div>
      </div>
