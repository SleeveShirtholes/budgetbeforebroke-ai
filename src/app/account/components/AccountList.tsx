 ...
 * @param {() => void} onCreateAccount - Callback function when creating a new account
 * @param {boolean} isLoading - Whether the accounts are currently loading
 */
export default function AccountList({
  ...
  const uniqueAccounts = Array.from(
    new Map(accounts.map((acc) => [acc.id, acc])).values(),
  );
  return (
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-gray-500">
            ...
            {uniqueAccounts.length === 1 ? "Account" : "Accounts"}
          </div>
        </div>
