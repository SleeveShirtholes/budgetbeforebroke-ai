...
import { user, transactions, categories, budgets, budgetCategories, budgetAccounts } from '@/db/schema';
...
export async function processSmsMessage(smsData: SmsData): Promise<string> {
  ...
  if (trimmedMessage.startsWith('budget') || trimmedMessage.startsWith('balance')) {
    return await handleBudgetQuery(userData.id, userData.defaultBudgetAccountId, trimmedMessage);
  }
  
  // Handle transaction commands
  if (trimmedMessage.startsWith('spent') || trimmedMessage.startsWith('expense') || 
      trimmedMessage.startsWith('paid') || trimmedMessage.startsWith('bought') ||
      trimmedMessage.startsWith('income') || trimmedMessage.startsWith('earned') ||
      trimmedMessage.includes('$') || trimmedMessage.match(/\d+(\.\d{2})?/)) {
    return await handleTransactionCommand(userData.id, userData.defaultBudgetAccountId, message);
  }
...
• Earned $100 side hustle`;
  ...
  try {
    ...
    if (transactionData.category) {
      categoryId = await findOrCreateCategory(budgetAccountId, transactionData.category);
    }
    ...
    const budgetInfo = categoryId ? await getBudgetRemainingForCategory(budgetAccountId, categoryId) : null;
...
async function handleBudgetQuery(
  ...
  try {
    ...
    const budgetIndex = words.findIndex(word => word === 'budget' || word === 'balance');
    const categoryName = budgetIndex >= 0 && words[budgetIndex + 1] ? words[budgetIndex + 1] : null;
    
    if (categoryName) {
      // Query specific category
      const categoryBudget = await getCategoryBudgetInfo(budgetAccountId, categoryName, currentYear, currentMonth);
      if (!categoryBudget) {
...
Remaining: $${remaining.toFixed(2)}${remaining < 0 ? ' (over budget)' : ''}`;
      ...
      const allBudgets = await getAllCategoryBudgets(budgetAccountId, currentYear, currentMonth);
...
function parseTransactionMessage(message: string): TransactionData | null {
  ...
  description = description.replace(/^(spent|paid|bought|expense|income|earned|received|got|deposit|on|for|at|from)\s*/i, '').trim();
 ...
 */
async function findOrCreateCategory(budgetAccountId: string, categoryName: string): Promise<string> {
 ...
 */
async function getBudgetRemainingForCategory(budgetAccountId: string, categoryId: string) {
  const currentDate = new Date();
  const currentYear = currentDate.getFullYear();
  const currentMonth = currentDate.getMonth() + 1;
 ...
 */
async function getAvailableCategories(budgetAccountId: string): Promise<string> {
