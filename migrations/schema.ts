import { pgTable, foreignKey, text, numeric, date, boolean, timestamp, unique, integer } from "drizzle-orm/pg-core"
...
export const budgetAccountMember = pgTable("budget_account_member", {
	id: text().primaryKey().notNull(),
	budgetAccountId: text("budget_account_id").notNull(),
	userId: text("user_id").notNull(),
	role: text().notNull(),
...
export const budget = pgTable("budget", {
	...
	unique("budget_budget_account_id_year_month_unique").on(table.budgetAccountId, table.year, table.month),
]);
...
export const contactSubmission = pgTable("contact_submission", {
	...
	lastSupportMessageAt: timestamp("last_support_message_at", { mode: 'string' }),
}, (table) => [
	foreignKey({
...
export const monthlyDebtPlanning = pgTable("monthly_debt_planning", {
	...
	unique("monthly_debt_planning_budget_account_id_debt_id_year_month_uniq").on(table.budgetAccountId, table.debtId, table.year, table.month),
]);
...
export const budgetAccountInvitation = pgTable("budget_account_invitation", {
	id: text().primaryKey().notNull(),
	budgetAccountId: text("budget_account_id").notNull(),
...
export const supportComment = pgTable("support_comment", {
	id: text().primaryKey().notNull(),
...
export const transaction = pgTable("transaction", {
	...
	unique("transaction_plaid_transaction_id_unique").on(table.plaidTransactionId),
]);
